"""
    ...
    the CSV used for this file is called "ThorRangarokTwitterData.csv"
...
FILE_NAME = "ThorRangarokTwitterData.csv"
SHOW_OFFER = True 
...
def main():
    try:
        ...
        ch.queue_delete(queue="Contributors")
        ch.queue_delete(queue="Coordinates")
        ch.queue_delete(queue="Created_at")

        # use the channel to declare a durable queue
        # a durable queue will survive a RabbitMQ server restart
        # and help ensure messages are processed in order
        # messages will not be deleted until the consumer acknowledges
        ch.queue_declare(queue="Contributors", durable=True)
        ch.queue_declare(queue="Coordinates", durable=True)
        ch.queue_declare(queue="Created_at", durable=True)
        ...
        with open(FILE_NAME, "r") as input_file:
            ...
            for row in reader:
                ...
                send_message("Contributors", first)
                send_message("Coordinates", second)
                send_message("Created_at", third)
